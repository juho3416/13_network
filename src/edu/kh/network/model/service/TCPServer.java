...
public class TCPServer {
	...
	// --> 서버소켓과 클라이언트소켓이 연결되어 데이터통신이 가능해짐.
...
//		1. 서버의 포트번호 정함
		...
		OutputStream os = null; // 서버 -> 클라언트 출력용 스트림 변수
...
//			3. 클라이언트 쪽에서 접속 요청이 오길 기다림
			// - 서버용 소켓은 생성되고 나면 클라이언트 요청이 오기 전까지
...
//			7. 스트림을 통해 읽고 쓰기
			...
			Date now = new Date(); // 생성된 시간을 기록하고 있는 시간 관련 객체 (java.util에서 import)
			//  -> 날찰 출력 형식 변경이 필요함
